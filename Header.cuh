...
__global__ void _customFillDTensor(float *dTensor, uint32_t size, uint32_t width, uint32_t seed1, uint32_t seed2) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= size) return;
    if (idx % width == 0) {
        dTensor[idx] = 1.0f;
...
void customFillDTensor(float *dTensor, uint32_t size, uint32_t width, uint32_t *seed1, uint32_t *seed2) {
    mixSeed(seed1, seed2);
    _customFillDTensor<<<(size >> 10) + (size & 0x3ff), 0x400>>>(dTensor, size, width, *seed1, *seed2);
}
...
__global__ void _integratedAdamUpdate(float *dTensor, float *dTensorGrad, float *dTensorMean, float *dTensorVar, float betaMean, float betaVar, float betaMeanCor, float betaVarCor, float learningRate, float weightDecay, uint32_t size) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= size) return;
...
void integratedAdamUpdate(float *dTensor, float *dTensorGrad, float *dTensorMean, float *dTensorVar, float betaMean, float betaVar, float betaMeanCor, float betaVarCor, float learningRate, float weightDecay, uint32_t size) {
    _integratedAdamUpdate<<<(size >> 10) + (size & 0x3ff), 0x400>>>(dTensor, dTensorGrad, dTensorMean, dTensorVar, betaMean, betaVar, betaMeanCor, betaVarCor, learningRate, weightDecay, size);
}
